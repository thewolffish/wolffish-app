...
// Standalone overlay shown while a PROCEDURE runs. A procedure runs through the
// same brainstem single-flight queue as a heartbeat automation, so it surfaces
// on the same window.api.heartbeat.* run channel — this is a deliberate replica
// of HeartbeatActiveOverlay with its own icon + copy, mounted only when the
// running job is a procedure (id prefixed "procedure:"; see App.tsx useActiveRun).
      ...
      <div className="flex w-full max-w-md flex-col items-center gap-6 px-6">
        <div className="relative">
          ...
          <div className="relative flex h-10 w-10 items-center justify-center rounded-full bg-emerald-500/15">
            <PlayIcon size={20} className="text-emerald-500 animate-pulse" />
          </div>
        </div>
        ...
        <p className="text-muted/50 text-center text-[11px]">{t('procedures.overlay.blocked')}</p>
      </div>
    </main>
  )
}

 ...
 * the engine behind the `telegram_check_format` tool. This does NOT
 * rewrite or repair anything (the model is the formatter); it only reports
 * exactly what Telegram's parser would reject, so the model can fix its own
 * markup before calling `telegram_send`. Catches the four failure modes
 * that produce a 400 "can't parse entities":
 *   1. a tag Telegram doesn't support (a stray `<message>`, a leaked
 *      `<p>`/`<br>`/`<h2>`, a Markdown-ish tag),
 *   2. an unclosed tag (`<b>` with no `</b>`),
 *   3. a mismatched/orphan closing tag (`</i>` with no open `<i>`),
 *   4. a bare `<` or `&` that should have been written `&lt;` / `&amp;`.
 * `>` is left unchecked — Telegram accepts a bare `>` in text.
 */
export function validateTelegramHtml(message: string): { ok: boolean; issues: string[] } {
  ...
  if (bareLt > 0) {
    issues.push(`${bareLt} bare "<" not starting a valid tag — write a literal less-than as &lt;.`)
  }
  if (bareAmp > 0) {
    issues.push(`${bareAmp} bare "&" not part of an entity — write a literal ampersand as &amp;.`)
  }
  ...
  if (/\*\*[^*\n]+\*\*/.test(message)) {
    issues.push('Markdown **bold** — Telegram shows the asterisks literally. Use <b>bold</b>.')
  }
  if (/^\s{0,3}#{1,6}\s+\S/m.test(message)) {
    issues.push('Markdown "# heading" — Telegram has no headings. Use a short <b>bold</b> line.')
  }
  if (/!?\[[^\]\n]*\]\((?!wolffish-media:\/\/)[^)\s]+\)/.test(message)) {
    issues.push('Markdown [text](url) link — Telegram shows it raw. Use <a href="url">text</a>.')
  }
  ...
  if (/^\s{0,3}([-*_])(?:\s*\1){2,}\s*$/m.test(message)) {
    issues.push(
      'Markdown "---" rule — Telegram shows it raw. Use a blank line to separate sections.'
    )
  }
